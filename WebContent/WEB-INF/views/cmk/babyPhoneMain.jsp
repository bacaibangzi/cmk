...
<body>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>babyPhone/list.htm?entityId=${vo.entityId}";
			...
			tableObj.aoColumns=[
			    ...
			    {"mDataProp":"babyName","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"100"},
			    {"mDataProp":"babyPhone","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"200"}
			];
			//添加额外的参数babyName
		...
		function update()
		{
			...
			window.location = "<%=basePath%>babyPhone/eidt.htm?entityId="+selectId; 
		...
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>babyPhone/detail.htm?entityId=' + selectId);
		...
		function highQuery()
		{
			// 查询条件 区域名称
			var areaName_filter = '';
			var returnVal = '<%=basePath%>babyPhone/list.htm?orgCode=${sessionScope.accountInfo.orgCode}';
			if(areaName_filter!=''){
		...
		function deleteAll()
		{
			...
			if(rows.length!=1){
				...
				var actionUrl = "<%=basePath%>babyPhone/delete.htm?code=${vo.code}&entityIds=";
