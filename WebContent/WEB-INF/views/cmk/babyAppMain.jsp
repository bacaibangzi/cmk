...
<body>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>babyApp/list.htm?entityId=${vo.entityId}";
			...
			tableObj.aoColumns=[
			    ...
			    {"mDataProp":"extend1","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"100"},
			    {"mDataProp":"appName","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"200"}
			];
			//添加额外的参数DateString
		...
		function update()
		{
			...
			window.location = "<%=basePath%>babyApp/eidt.htm?entityId="+selectId; 
		...
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>babyApp/detail.htm?entityId=' + selectId);
		...
		function highQuery()
		{
			// 查询条件 区域名称
			var areaName_filter = '';
			var returnVal = '<%=basePath%>babyApp/list.htm?orgCode=${sessionScope.accountInfo.orgCode}';
			if(areaName_filter!=''){
		...
		function deleteAll()
		{
			...
			if(rows.length!=1){
				...
				var actionUrl = "<%=basePath%>babyApp/delete.htm?code=${vo.code}&entityIds=";
