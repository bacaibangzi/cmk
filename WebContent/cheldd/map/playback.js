...
var PlayBack = function(mapId, user_id, productType, sudu, url, ip) {
...
PlayBack.prototype.getDataFrist = function(from, to, q, ip, url) {
	if (ip == 'null') {
		alert("当前未有轨迹数据!");
		return
	};
...
PlayBack.prototype.playMarker = function(array) {
	if (PlayBack.FRIST_LOAD) {
		...
		PlayBack.MARKERS_OBJ = new PopupMarker({
			position: point,
			map: PlayBack.map,
			icon: PlayBack.ZY + "cheldd/map/green.gif",
			text: txt,
			showpop: true
		});
		...
		if (PlayBack.HISTORY_PLAY_FLAG) {
			if (i < array.length) {
				if (array[i][3] > PlayBack.overSpeed) {
					var color = "#00FF00";
					if (array[i][3] > 1.5 * PlayBack.overSpeed) color = "#00FF00";
...
PlayBack.prototype.ajaxRequest = function() {
	...
	var parms = "?sn=" + PlayBack.IP + "&from=" + PlayBack.NEXT_TIME + "&to=" + PlayBack.TO_TIME;
	...
	var url = PlayBack.AJAX_URL + parms;
	//alert(url);
	...
	oXHR.onreadystatechange = function() {
		if (oXHR.readyState == 4) {
			if (oXHR.status == 200 || oXHR.status == 304) {
				...
				PlayBack.getDataCallBack(oXHR.responseText)
			}
		}
	}
};
PlayBack.prototype.getTxtByRecord = function(record) {
	var html = [];
	html.push('<font>');html.push('<span style="font-weight:bold;">'+lg.carSpeed+':</span>' + record[3] + lg.kPerH + '<br/>');
	html.push('<span style="font-weight:bold;">'+lg.carMovement+':</span>' + formatKm2M(PlayBack.DISTANCE) + '<br/>');
	html.push('<span style="font-weight:bold;">'+lg.carGps+':</span>' + utcToLocal(record[2]) + '<br/><br/>');
	
	//html.push('<font>');html.push('<span style="font-weight:bold;"> :</span> <br/>');
	//html.push('<span style="font-weight:bold;"> :</span> <br/>');
	//html.push('<span style="font-weight:bold;"> :</span> <br/>');
...
PlayBack.prototype.getTxtByCount = function() {
	...
	html.push('<span style="font-weight:bold;">' + lg.idle + ':</span>' + exchangeTime(total - PlayBack.RUN_TIME) + '<br/><br/>');
	html.push('</font>');
	return html.join("")
};
